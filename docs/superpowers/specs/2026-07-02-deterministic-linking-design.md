...
**Messergebnis (2026-07-04):**

- **Lauf 1** (`apply-link-rules.mjs` auf einer Kopie `knowledge-graph.measure.json` im MachineSIC-`.understand-anything`, damit die Projektwurzel-Ableitung auf die echten Quelldateien zeigt): Exit 0, Summary `rules=7 files=164 added=21 upgraded=3 skippedRules=0 skippedEdges=0`. Alle 7 Regeln der drei Packs (wpf 3, razor 2, dryioc 2) geladen; 164 Dateien geparst (147 `.cs` + 9 `.xaml` + 8 `.razor`). **Keine** `Warning:`-Zeile — kein Regel-/Grammatik-/Knoten-Degradationspfad wurde betreten; die in der Task-Erwartung genannten unauflösbaren DryIoc-/Razor-Typen traten nicht auf (die in-scope Razor-`@inject`-Typen lösten alle auf, und die DryIoc-Registrierungen liegen sämtlich außerhalb des Analyse-Scopes, s. u.).
- **Verteilung je Regel-ID:** `wpf.code-behind`=9, `wpf.xmlns-viewmodel`=5, `wpf.event-handler`=1, `razor.inject`=7, `razor.component-tag`=2, `dryioc.implements`=0, `dryioc.registration`=0. Summe **24** `rule`-Kanten = 21 neu + 3 Upgrades.
- **Code-behind-Paarung 9/9** (Messgröße §3 erfüllt, gemessen, nicht erzwungen): Die 9 `wpf.code-behind`-Kanten (`.xaml.cs` —`implements`→ `.xaml`, confidence 1.0) treffen mengengleich exakt die 9 `.xaml`-Datei-Knoten des Graphen — keine View ohne Paarung, kein Ziel außerhalb der View-Menge. Die 9 `.xaml`-Knoten sind 8 Dateien unter `TFS.SIC.LocalAdministration` — `App.xaml` (Application-Definition), `Style/CommonStyle.xaml` (ResourceDictionary) und die sechs Views `LogsView`, `MainWindowView`, `MessageHistoryView`, `ModuleInstanceView`, `ModulesDefinitionView`, `StatusBarView` — plus 1 Datei unter dem Installer (`TFS.Sic.Host.Installer/Visuals/SicHostConfigContentView.xaml`), also 8 + 1 = 9; alle tragen ein `x:Class`-Attribut, dessen Wert mengengleich einer `partial class`-FQN im gleichnamigen `.xaml.cs` entspricht, und paaren daher mechanisch.
- **Event-Handler-Kanten > 0** (Messgröße §3 erfüllt): 1 `wpf.event-handler`-Kante (`SicHostConfigContentView.xaml` —`calls`→ `.xaml.cs`, `Click="OnSuggestedPortClick"`, confidence 0.9). Das ist die einzige XAML-Code-behind-Event-Attribut-Verdrahtung im in-scope Bestand; die übrigen acht `.xaml` deklarieren keine Code-behind-Event-Attribute (Interaktion läuft über MVVM-`{Binding}`-Commands, die der generische Join korrekt **nicht** als Methodennamen trifft).
- **Razor:** 7 `razor.inject` + 2 `razor.component-tag` = 9 `depends_on`-Kanten über die Blazor-Komponenten in `TFS.SIC.LocalAPI`. **DryIoc: 0** — ehrlicher Nullbefund, kein Regelfehler: die zwei-Typargument-`Register<IService, TImpl>()`-Kompositionswurzeln (ExampleModule, Mp12NModule, ModuleSicModule, Bootstrapper*) liegen samt der gesamten `SIC.Old`-Referenz unter `/src/reference/`, das `.understandignore` vom Scan ausschließt (0 Referenz-Datei-Knoten im Graphen). Im in-scope Code existieren nur zwei `RegisterInstance`-Aufrufe in `HostConnectionHandler.cs` (einer ohne, einer mit **einem** Typargument `RegisterInstance<IApiClientResolver>`) — kein `(serviceFqn, implFqn)`-Paar, daher per §8.3/§8.7 bewusst kein Fakt. Die übrigen `Register`-Vorkommen sind `DependencyProperty.Register` (WPF-Dependency-Property, kein DI) und SignalR-`Invoke("Register", …)`.
- **Upgrade-Zähler 3:** drei `razor.inject`-Kanten stuften vorbestehende origin-lose Kanten auf `rule` hoch (`ModuleDetail.razor`→`SicHypervisor.cs`, `Settings.razor`→`SicHypervisor.cs`, `Settings.razor`→`UiStateService.cs`); die übrigen 21 Kanten sind Neuanlagen. Kein WPF-Upgrade: die vom LLM in §1 georateten ViewModel-Kanten trugen andere `(source, target, type)`-Tripel als die deterministischen `wpf.xmlns-viewmodel`-Kanten (`App.xaml`→`*View.xaml.cs`), sodass diese fünf neu angelegt statt hochgestuft wurden.
- **Invarianten `structural`/`manual` je `unchanged: true` — vacuously:** Der Prüfstein ist ein Pre-Provenance-Graph (alle 920 Ausgangskanten ohne `origin`; 0 `structural`- und 0 `manual`-Kanten, Count vor/nach je 0→0). Die Invariante „0 veränderte `structural`/`manual`-Kanten" hält hier also mangels solcher Kanten trivial, **nicht** weil der Linker gegen einen Graphen mit solchen Kanten getestet wurde. `apply-link-rules` lässt nicht-gematchte origin-lose Kanten unangetastet; den Default-Stempel (`origin: "llm"`) setzt erst der nachgelagerte `apply-graph-patches`-Schritt (§7.3), der in dieser isolierten Linker-Messung nicht mitlief.
- **Idempotenz byte-identisch:** Lauf 2 auf dem bereits verlinkten Graphen meldet `added=0 upgraded=0`; `cmp` bestätigt Lauf-1-Ausgabe == Lauf-2-Ausgabe (f(f(x))==f(x) gilt bereits nach dem ersten Lauf) und Lauf-2-Ausgabe == Lauf-3-Ausgabe byte-identisch.
- **Stichprobe 8/8 korrekt:** deterministisch jede 3. der 24 `rule`-Kanten (Indizes 0,3,6,9,12,15,18,21 in Graph-Reihenfolge), alle 5 feuernden Regeln abgedeckt, jede Kante gegen **beide** Quelldateien geprüft: `razor.inject` 2× (`@inject SicHypervisor Hypervisor` in `.razor` ↔ `public class SicHypervisor` in `SicHypervisor.cs`), `razor.component-tag` (`<Routes … />` in `App.razor` ↔ Komponente `Routes.razor`), `wpf.code-behind` 3× (`x:Class`-Wert == `namespace`+`partial class`-FQN; `SicHostConfigContentView`, `LogsView`, `ModuleInstanceView`), `wpf.event-handler` (`Click="OnSuggestedPortClick"` in `.xaml` ↔ `private void OnSuggestedPortClick(object, RoutedEventArgs)` in `.xaml.cs`), `wpf.xmlns-viewmodel` (`xmlns:views="clr-namespace:TFS.SIC.LocalAdministration.Views"` + `<views:ModuleInstanceView />` in `App.xaml` ↔ Klasse `ModuleInstanceView` in `ModuleInstanceView.xaml.cs`). Keine Ausreißer.
- **Sicherheit:** Der Original-`knowledge-graph.json` blieb byte-identisch (SHA-256 vor und nach allen Läufen unverändert); sämtliche Messläufe liefen auf der Kopie `knowledge-graph.measure.json`, die nach der Messung entfernt wurde.
