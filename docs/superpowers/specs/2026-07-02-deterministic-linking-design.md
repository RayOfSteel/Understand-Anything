...
- **Code-behind-Paarung 9/9** (Messgröße §3 erfüllt, gemessen, nicht erzwungen): Die 9 `wpf.code-behind`-Kanten (`.xaml.cs` —`implements`→ `.xaml`, confidence 1.0) treffen mengengleich exakt die 9 `.xaml`-Datei-Knoten des Graphen — keine View ohne Paarung, kein Ziel außerhalb der View-Menge. Die 9 `.xaml`-Knoten umfassen neben den sieben `LocalAdministration`-Views auch `App.xaml` (Application-Definition) und `Style/CommonStyle.xaml` (ResourceDictionary) sowie den Installer-`SicHostConfigContentView.xaml`; alle tragen ein `x:Class`-Attribut, dessen Wert mengengleich einer `partial class`-FQN im gleichnamigen `.xaml.cs` entspricht, und paaren daher mechanisch.
